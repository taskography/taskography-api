...
class PDDLGymDataset:
    def __init__(
        ...
        assert (
            train_scenes <= SPLIT_SCENES[split]
        ), "Number of training scenes exceeds dataset split size."
    ...
    def generate(
        ...
        pddlgym_domain = PDDLDomainParser(
            sampler_kwargs["domain_filepath"],
        ...
        for i, scene_graph_filepath in enumerate(scene_graph_filepaths):
            ...
            # Sample tasks
            if sampler.valid():
                if self.save_samplers:
                    sampler.save(dir=self.sampler_dir)
